...
def FoodData_preprocessing():
    ...
    filename = "./FoodData_Pedro/PairwiseRates_Food.csv"
    with open(filename, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        header = ["User_ID", "Item1_ID", "Item2_ID", "PairwiseScore"]
        writer.writerow(header)
    ...
    PairwiseRates = pd.read_csv("./FoodData_Pedro/PairwiseRates_Food.csv")  # .to_numpy()
    # Item_pairs is a list containing paired items
    ii, iii = PairwiseRates.shape
    Item_pairs = []
    for i in range(ii):
...
def mf(R, k, n_epoch=5000, lr=.0003, l2=.04):  # n_epoch=5000, lr=.0003
    ...
    for epoch in range(n_epoch):
        ...
        pd.DataFrame(P).to_csv('./FoodData_Pedro/User_Embedding.csv', header=True, index=True)
        pd.DataFrame(Q).to_csv('./FoodData_Pedro/PairItems_Embedding.csv', header=True, index=True)       
    return P, Q
